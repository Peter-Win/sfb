...
describe('Ship', () => {
	...
	it('buildFireTargets', () => {
		...
		expect(trace0.arcMap).to.be.equal(FiringArc.arc.FA)
	...
	it('onDamage', () => {
		...
		const res1 = ship.onDamage(game, droneC, null, 6)
		expect(res1).to.be.eql({shield: 6})
		expect(ship.shield[0]).to.be.equal(10)
	})
	it('buildLaunchTargets', () => {
		const game = new Game()
		game.create(scenario03())
		const ship = game.getShip('Main')
		const list = ship.buildLaunchTargets(game)
		expect(list).to.have.lengthOf(4)
		expect(list[0].devId).to.be.equal('DRN')
		expect(list[0].targetId).to.be.equal('freighterA')
		expect(list[1].targetId).to.be.equal('freighterB')
	})
})
