...
const FiringArc = Object.freeze({
	...
	inSector(arc, center, target) {
		const arcMapSrc = FiringArc.arc[arc]
		const arcMapAbs = FiringArc.rotateArc(arcMapSrc, center.dir)
		return FiringArc.testArc(arcMapAbs, center, target)
	},
	...
	testArc(arcMap, center, target) {
		...
		for (let sectorIndex = 0; sectorIndex < 6; sectorIndex++, mask <<= 1) {
			...
			if (mask & arcMap) {
				...
				if (FiringArc.testPart[sectorIndex](snow, target)) {
					return true
				}
			}
		}
...
module.exports = {FiringArc, Snowflake}
